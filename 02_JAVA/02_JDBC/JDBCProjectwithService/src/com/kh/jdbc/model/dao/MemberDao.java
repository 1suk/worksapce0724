...
public class MemberDao {
   ...
   public int insertMember(Connection conn, Member m) {
      ...
      /*
       * JDBC 사용순서
       * 1) JDBC Driver등록 : JDBC 내의 다양한 인터페이스가 특정 DBMS가 제공하는 클래스에 의해서 구현됨.(사용할 수 있도록 등록)
       * 2) Connection 생성 : 연결하고자하는 db정보를 입력해서 해당 db와 연결할 수 있는 객체 생성
       * 3) [Prepared]Statement : Connection 객체를 이용해서 Statement 객체 생성, sql문을 실행하고 결과를 받아주는 객체
       * 4) sql문을 전달해서 실행 : Statement 객체를 이용해서 sql문 실행
       * 5) 결과받기 -> select문 실행 -> ResultSet 객체(조회된 결과를 담아줌) -> 6_1
       *           -> DML(insert, update, delete) -> 처리된 행 수 -> 6_2
       * 6_1) ResultSet 객체에 담겨있는 데이터를 하나씩 추출해서 자바메모리에 담아 사용
       * 6_2) 트랜젝션 처리(성공했으면 commit, 실패시 rollback 실행)
   ...
   public int deleteMembers(Connection conn, List<String> userIds) {
	    int result = 0;
	    PreparedStatement pstmt = null;
   ...
   public ArrayList<Member> searchKeywordList(Connection conn, String userName) {
	    ResultSet rset = null;
	    ArrayList<Member> list = new ArrayList<>();
	    PreparedStatement pstmt = null;

	    String sql = "SELECT * FROM MEMBER WHERE USER_NAME LIKE '%' || ? || '%'";
	    //SELECT * FROM MEMBER WHERE USER_NAME LIKE '%?%' (X) -> "%'?'%"
	    try {
	        pstmt = conn.prepareStatement(sql);
