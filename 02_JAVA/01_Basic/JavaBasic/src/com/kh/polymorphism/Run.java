...
public class Run {
	...
	public static void main(String[] args) {
		...
		//c1.sweet();
		//오버라이딩시에는 실제 메로리에 있는 재정의된 메서드가 호출됨.
		//c1.yummy();
		//다운캐스팅 : 자식의 참조변수로 타입을 변경(업캐스팅관계에 있을 경우에만 가능)
		((CheeseCake)c1).milky();
		
		//CheeseCake c2 = new Cake();
		//실제 메로리가 Cake이므로 ChesseCake 공간에 접근할 수 없음
		//-> 접근범위가 실제 메모리의 크기보다 클 수 없음 
		
		//1. 부모타입의 래퍼런스(참조변수)로 부모객체를 다루는 겨우
		//Car c1 = new Car("빨간색","가솔린",2021);
		//c1.drive();
		//((Sonata)c1).moveSonata(); -> Car c1 = new Sonata("빨간색","가솔린",2021) 이면 가능
		
		//2. 자식타입 래퍼런스 자식객체를 다루는 경우
		Avante c2 = new Avante("흰색","LPG",2022);
		c2.drive(); //Avante클래스에서 오버라이딩된 drive 호출
		c2.moveAvante();
		//부모참조변수로 업캐스팅 가능 -> 오버라이딩된 메서드는 생선된 메모리를 기준으로 호출됨.
		((Car)c2).drive(); //업캐스팅시 자식요소의 메로리 접근이 불가.
		//(Car)c2).moveAvante();
