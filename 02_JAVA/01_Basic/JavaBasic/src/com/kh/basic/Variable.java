...
public class Variable {
	...
	public static void main(String[] args) {
		...
		//실수형 (소수점 6자리까지 사용) 자료형으로 num2라는 이름의 변수를 선언
		float num2;
		num2 = 3.1415926974f;
		 ...
		 * */
		int num3 = 10;
		byte by = 10; //10(int)가 by값보다 크지만 해줌
		short sh = 10;
		long lo = 10;
		 ...
		 * 문자열 (객체 타입)
		 * String
		 * 문자열 같은 경우 할당할 메모리크기가 가변적이르므로 원시타입이 아닌 객체타입으로 변수를 만들어 사용함.
		 * 
		 * 원래 객체를 만드는 방식
		...
		String str1 = "안녕하세요. 장원석입니다.";
		System.out.println(str1);
		
		String str2;
		str2 = "누구?";
		str2 = "저는 누구가 아니라 장원석입니다.";
		...
		/*
		 * 이스케이프 시퀀스
		 * 문자열 내에서 탭, 백슬러시, 작따옴표등을 사용하기위한 방식
		 * */
		...
		// \\: 백슬래시(\)
		// 문자 자체로 출력, 두개 써야 하나로 나옴
		System.out.println("경로 : C:\\workspace\\02_JAVA ");
		 ...
		 * 프로그래밍에서 이름 짓는 방식
		 * 
		 * 카멜케이스 : 단어를 나열하며 이름을 짓되, 두번재 단어부터는 단어의 첫글자를 대문자로 시작 (자바에서 모든 메서드, 변수명)
		 * ex) userName, totalPrice
		 * 
		 * 파스칼케이스 : 카멜케이스에서 첫글자도 대문자로 작성(자바에서는 클래스명에 사용)
		 * ex) UserName, TotalPrice
		 * 
		 * 스네이크케이스 : 문자와 문자사이를 언더스코어(_)를 통해서 이어주는 것
		 * ex) user_name, total_price
		 * 
		 * 대문자스네이크케이스 : 스네이크케이스 + 모두 대문자 (대부분 언어에서 상수명)
		 * ex) USER_NAME
		 * 
		 * 케밥케이스 : 단어를 소문자로 작성화되 문자와 문자사이를 하이픈(-)으로 이어주는 것(html속성, url, 설정파일등)
		 * ex) user-name 
		 * */
	}
