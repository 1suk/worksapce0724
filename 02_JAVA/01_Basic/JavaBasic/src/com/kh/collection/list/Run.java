...
public class Run {
	...
	public static void main(String[] args) {
		//List(순서 O, 중복 허용)
		//ArrayList(배열기반), LinkedList(노드 기반), Vector, Stack(Vector 기반 LIFO 구조)
		...
		list.add(new Human("이무진", 25)); //toString 출력됨
		list.add("이무진"); //여러타입을 저장할 수 있음
		list.add(new Human("이창섭", 27));
		list.add(new Human("임재민", 29));
		...
		list.add(1, new Human("최지원", 29));
		//System.out.println(list);
		
		//3.remove(int index) : 해당 인덱스 위치의 데이터를 삭제 시켜주는 메서드
		list.remove(1);
		System.out.println(list);
		...
		//5.set(int index, E e) : 해당 인덱스의 값을 전달 받은 e객체로 덮어씌움
		...
		*/
		...
		//검색 search(E e) 데이터 검색 후 존재하면 위치반환(위에서부터 센다) -> equals를 사용하여 비교 검색
