...
SELECT E.EMP_NAME, E.SALARY, J.JOB_NAME
FROM EMPLOYEE E
...
--4. DEPARTMENT 테이블의 직원명, 이메일, 전화번호, 고용일 조회
SELECT EMP_NAME, EMAIL, PHONE, HIRE_DATE FROM EMPLOYEE;

--5. EMPLOYEE 테이블에서 이름, 연봉, 총수령액(보너스 포함), 실수령액(총수력액 - (연봉 * 세금(3%))
...
--사원명, 입사일, 근무일수를 EMPLOTEE 테이블에서 조회
--데이터베이스에서 날짜를 계산할 때 덧셈,뺄셈이 가능하다.
--현재시간 - 입사일 = 근무시간
--DATE - DATE => 결과를 무조건 일로 표시한다.
--코드 시행시 현재날짜를 표시하는 상수 : SYSDATE[년/월/일/시/분/초]
SELECT EMP_NAME, HIRE_DATE, (SYSDATE - HIRE_DATE) * 12 AS 근무시간
FROM EMPLOYEE;
...
SELECT EMP_NAME 사원명, SALARY AS 급여, NVL(BONUS,0) "보너스", SALARY * 12 AS "연봉"
FROM EMPLOYEE;
...
/*
SELECT DISTINCT EMP_NAME, JOB_CODE, DEPT_CODE
FROM EMPLOYEE;
*/
--위처럼 사용시 에러가 발생한다. DISTINCT는 한 명령어에서 한번만 사용가능
...
/*
    <WHERE 절>
    조회하고자 하는 테이블로부터 특정 조건에 만족하는 데이터만 조회하고자 할 때 사용함.
...
--EMPPYEE에서 부서코드가 'D1'인 사원들의 사원명, 급여, 부서코드 조회
SELECT EMP_NAME, SALARY,DEPT_CODE 
FROM EMPLOYEE
WHERE DEPT_CODE = 'D1';

--EMPPYEE에서 부서코드가 'D1'이 아닌 사원들의 사원명, 급여, 부서코드 조회
SELECT EMP_NAME, SALARY,DEPT_CODE 
FROM EMPLOYEE
WHERE DEPT_CODE != 'D1';

--월급이 400만원 이상인 사람들의 사원명, 부서코드, 급여 조회
...
/*
    ...
    2. '_' : 1글자를 대체검색할 때 사용
    EX) 비교할 대상 컬럼 LIKE '_문자' : 비교할 대상 컬럼 값 문자앞에 아무글자나 딱 한글자가 있는 값을 조회
        비교할 대상 컬럼 LIKE '문자_' : 비교할 대상 컬럼 값 문자뒤에 아무글자나 딱 한글자가 있는 값을 조회
        비교할 대상 컬럼 LIKE '_문자_' : 비교할 대상 컬럼 값 문자앞뒤에 아무글자나 딱 한글자가 있는 값을 조회
        
        비교할 대상 컬럼 LIKE '_문자__' : 내가 원하는 형태로 _를 통해서 문자수를 조절 할 수 있다.
*/
...
--사원들 중에서 전화번호가 3번째 자리가 1인 사원들은 사번, 사원명, 전화번호 조회
...
--2. 전화번호가 처음 3자리가 010이 아닌 사원들의 사원명, 전화번호 조회
...
--4. 부서테이블에서 해외영어부서인 부서들의 부서코드, 부서명 조회
...
SELECT EMP_NAME 사원명, SALARY AS 급여, NVL(BONUS,0) AS 보너스
FROM EMPLOYEE
...
--8. 급여가 200만원 이상이고 500만원 이하인 사원 중에서 입사일이 '01/01/01' 이상이고 보너스를 받지 않는 사원들의 사번, 사원명, 급여, 입사일, 보너스 조회
SELECT EMP_ID, EMP_NAME, HIRE_DATE,DEPT_CODE
FROM EMPLOYEE
WHERE HIRE_DATE >= '95/01/01' AND SALARY >= 2000000 AND SALARY <= 5000000 AND BONUS IS NULL;

--9.보너스를 포함 연봉((SALARY + (SALARY * BONUS)) * 12)이 NULL이 아니고 이름에 '하'가 포함된 사원들의 사번, 사원명, 급여, 보너스포함 연봉 조회
...
/*
    ...
    FROM 테이블
    WHERE 조건식
    ORDER BY 정렬기준이 될 컬럼 | 별칭 | 컬럼순번 [ASC|DESC] [NULLS FIRST | NULSS LAST]
