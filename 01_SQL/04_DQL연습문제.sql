...
SELECT EMP_NAME, SALARY * 12 AS 연봉, (SALARY + (SALARY * NVL(BONUS,0))) AS 총수령액,
(SALARY + (SALARY * NVL(BONUS,0))) - ((SALARY * 12) * 0.03) AS 실수령액
FROM EMPLOYEE;
...
SELECT EMP_NAME, FLOOR(HIRE_DATE - SYSDATE) AS 근무일수1 , FLOOR(SYSDATE - HIRE_DATE) AS 근무일수2
FROM EMPLOYEE;
...
SELECT EMP_NAME, CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE) / 12) AS 근무년수
FROM EMPLOYEE
