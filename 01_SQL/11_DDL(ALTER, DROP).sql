/*
    ...
    *변경할 내용
    ...
    데이터를 유지하면서 수정하고 싶을때 사용, 잘 사용할 일이 없음
*/
...
--1_2) 컬럼 수정
--데이터타입 수정 : MODIFY 컬럼명 변경하고자 하는 데이터타입
...
--1_3) 컬럼삭제(DROP COLUMN) : DROP COLUMN 삭제하고자하는 컬럼
...
ALTER TABLE DEPT_COPY 
    ADD CONSTRAINT DEPT_UQ_TITLE UNIQUE(DEPT_TITLE)
...
ALTER TABLE DEPT_COPY
    DROP CONSTRAINT DEPT_UQ_TITLE
    MODIFY LNAME NULL;
    
-------------------------------------------------------------------------------
--테이블 삭제
--DROP TABLE 테이블명;
--어딘가에 참고되고있는 부모테이블을 함부로 삭제가 되지 않는다.
--만약 지우고자한다면
...
--3)컬럼명/제약조건명/테이블명 변경
--3_1) 컬럼명 변경 : RENAME COLUMN 키존컬럼명 TO 새로운컬럼명
