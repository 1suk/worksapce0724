...
-------------------------------------------------------------------------------
/*
    1-2) 래퍼런스 타입 변수 : 어떤 테이블의 어떤 컬럼의 데이터타입을 참조해서 사용할지를 정하는 방식
*/
...
-------------------------------------------------------------------------------
/*
    래퍼런스타입 변수로 EID, ENAME, JCODE, SAL, ETITLE을 선언하고 
    각 자료형 EMPLOYEE(EMP_ID, EMP_NAME, JOB_CODE, SALARY), DEPARTMENT(DEPT_TITLE)을 참조하여
    사용자가 입력한 사번의 사번, 사원명, 직급코드. 급여, 부서명 조회 후 각 볌수에 담아서 출력
*/
DECLARE 
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
...
BEGIN 
    ...
    INTO EID, ENAME, JCODE, SAL, ETITLE
    FROM EMPLOYEE
    ...
    DBMS_OUTPUT.PUT_LINE('SAL : ' || SAL);
    DBMS_OUTPUT.PUT_LINE('ETITLE : ' || ETITLE);
END;
...
//보너스를 받지 않은 사원은 보너스를 지급받지 않는 사원입니다.
...
--일반 타입변수 TEAM을 문자열타입으로 생성하고 사용자가 입력한 사원정보를 가져와서
...
DECLARE 
    ...
    NCODE NATION.NATIONAL_CODE%TYPE;
    TEAM VARCHAR2(10);
BEGIN 
    ...
    DBMS_OUTPUT.PUT_LINE('부서명 : ' || NCODE);
