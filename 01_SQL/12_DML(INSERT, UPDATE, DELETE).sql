/*
    ...
    TCL(TRANSACTION 트랜잭스 제어어) : COMMIT, ROLLBACK
...
VALUES(900,'김수민', '880914-1234567', 'SD123@naver.com', 010123456789, 'D7', 'J5', 4000000, 0.2, 200, SYSDATE, NULL, 'N');

/*
    2. INSERT INTO 테이블명(컬럼, 컬럼, 컬럼...) VALUES(값,값,값..)
    테이블에 내가 선택한 컬럼에 대한 값만 INSERT 할 수 있다.
    컬럼값을 제시하지 않은 값은 기본적으로 NULL이 들어가면 DEFAULT가 설정되어있다면 DEFAULT값으로 설정됨
    -> DEFAULT가 없고 NOT NULL 제약조건이 걸려있는 컬럼은 반드시 직접 값을 넣어줘야한다.
*/
...
/*
    ...
    VALUES에 직접 값을 명싷는 것이 아니라, 대신 서브쿼리로 조화된 값을 모두 INSERT 가능
*/
...
--EMP_SALLARY 테이블에서 전체사원의 급여를 기존 급여에 10% 인상된 금액으로 변경
UPDATE EMP_SALARY
    SET SALARY = SALARY + (SALARY * 0.1);
    
/*
    UPDATE에서 SUBQUERY 사용가능
...
/*
    4.DELETE 
    테이블에서 기록된 데이터를 삭제하는 구문(한 행 단위로 삭제가 됨)
