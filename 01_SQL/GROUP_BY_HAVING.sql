...
--각 부서별 사원수
SELECT DEPT_CODE, COUNT(*), SUM(SALARY)AS 월급 --3
FROM EMPLOYEE --1
...
--남자사원과 여자사원의 수를 조회
SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1','남','2','여'), COUNT(*) --3
FROM EMPLOYEE --1
GROUP BY SUBSTR(EMP_NO, 8, 1); --2

--GROUP BY절에 여러컬럼을 기술할 수 있음
--GROUP BY DEPT_CODE; 경우 -> JOB_CODE SELECT 불가
...
--WHERE절이 GROUP BY 이전에 실행되므로 해당 코드는 300만원 이상의 급여를 받는 사람을 먼저 필터링한 후 부서코드와 평균을 구한다.
SELECT DEPT_CODE, ROUND(AVG(SALARY))
FROM EMPLOYEE
...
--직급별 직급코드, 총 급여합(단, 직급별 총급여합이 100만원 이상인 직급만 조회)
