...
-- 서브쿼리 예시2)
--전 직원의 평균급여 보다 더 많은 급여를 받는 사원들의 사번, 이름, 직급코드, 급여 조회
...
/*
    ...
    비교대상 >, < ANY(값~) : 여러개의 결과값 중 한개라도 비교 연산을 만족하는가?
    비교대상 >, < ALL(값~) : 여러개의 결과값 모두가 비교 연산을 만족하는가?
*/
...
--1) 하이유 사원과 같은 부서코드, 같은 직급코드에 해당한느 사원들 조회
--사원명, 부서코드, 직급코드, 입사일
...
--> 인라인뷰를 통해서 TOP-N 분석 : 상위 몇개만 조회
--전 직원 중 급여가 가장 높은 직원 5명
SELECT EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY DESC;

--> ORDER BY절이 수행된 결과를 가지고 ROWNUM을 부여
...
FROM (SELECT EMP_NAME, SALARY, HIRE_DATE FROM EMPLOYEE ORDER BY HIRE_DATE DESC) WHERE ROWNUM <= 5;
WHERE ROWNUM <= 5
